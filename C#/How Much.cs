...
public class Carboat {
    ...
    return arraysToValues(computePossibleFortunes(m,n));
	}
  
   static List<int[]> computePossibleFortunes(int m, int n) {
   List<int[]> possibleFortunes = new List<int[]>();
   int max = Math.Max(m, n);
   int min = Math.Min(m, n);
   int[] boatsandcars= new int[3];   
   ...
   for (int i = min; i <= max; i++) {
      if (i % 9 == 1 && i % 7 ==2)  {
       ...
       boatsandcars[2] = i/9;
       boatsandcars[1] = i/7;
       arraycopy = (int[]) boatsandcars.Clone();
       possibleFortunes.Add(arraycopy);
     }
    }
    return possibleFortunes;
   }
  
  static string arraysToValues(List<int[]> values) {
  StringBuilder output = new StringBuilder("[");
    
  for (int i=0; i<values.Count; i++)
    output.Append("[M: " + values[i][0] + " B: " + values[i][1] + " C: " + values[i][2] + "]");
   output.Append("]");
